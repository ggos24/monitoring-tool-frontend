...
- `!` contested chip (when `is_contested=true` — current v6 verdict `contested_signals`, Decision 34): 9px mono, amber-950 bg / amber-300 text / amber-800 border (same trio as the "mixed" sentiment pill). Marks domains where independent sources disagree (Forbes, Bild, Fox News…) — the editorial-review cue, visible in every view.
- **Panel-local toggles** (row under the subtitle, MiniToggle pattern: 9px mono kicker label + bordered segmented control, active segment `bg-foreground text-background`): `SORT Ment./Priority` and `VIEW T+R/Combo`. Sort state lives on the Overview page and couples BOTH panels: `/api/stats/sources?sort=priority` for the panel and `/api/mentions?sort=priority` for the list (mentions ordered by source standing: trust band → domain reach DESC → recency; "sorted by source priority" note appears next to the Mentions kicker). Sort=priority calls `/api/stats/sources?sort=priority` — trust-gated, reach-ranked "who to watch" order (trusted first, then unvetted → low → propaganda last; reach DESC inside each band). Subtitle switches to "by priority (trust-gated, reach-ranked)".
- **Combo view (experimental prototype):** replaces the two Tr/Re badges with ONE `TrustReachComboBadge` — colour = trust band (same red→green ramp), digit = reach tier ("—" dim when unresolved). Header column `T·R`. Semantics guard from Decision 32 g2 holds: colour stays the gate verdict, so high-reach propaganda is a red badge with a big digit — loud, never "good". Native title only (no rich popover) while prototype.
...
- ReachBadge (REACH, Decision 32): 18×18 sharp square with the reach tier 0–5. With a `domain` prop it now opens its own rich hover popover (mirrors the trust one, same cached `/api/scoring/news_domain/{domain}` query — hovering both badges costs one request): band + 0-100 score, the 50/50 authority/traffic formula split, raw reach signals (InLink Rank n/1000, est. organic visits/mo, Tranco global rank), refreshed date, and the reminder that reach measures loudness, never quality. **Deliberately a different ramp** — a monochrome **blue "intensity"** scale (`bg-strong` at 0 → `bg-sky-950…600` for 1–5), NOT the red→green trust semantics, so a high-reach propaganda outlet never reads as "good". Reach = how *loud* a source is (audience/authority), not how trustworthy. `null` (no DataForSEO/Tranco signal yet) → dim `—`. Native tooltip: `Reach: N/5 · <band> · <score>/100`. `reach_score` (0-100) is the sort key; the badge shows the derived `reach_tier`.
- **Popover stacking:** every `Tooltip.Positioner` (trust popover, reach popover, country flag, stance pills) carries `z-[100]` — the positioner's transform creates a stacking context, so a z-class on the inner Popup alone loses to the panel's sticky column header (`z-10`); same precedent as the CountryFilter combobox.
...
- Filter bar at top — **two rows**:
  ...
  - **Row 2:** `QUALITY` ToggleGroup (All / Trusted / Suspect / Unvetted / Contested / Propaganda) + `STANCE` ToggleGroup (All / Supportive / Critical / Neutral / Mixed). `Unvetted` (trust=2 only, Decision 33) is a subset of `Suspect` (trust 1–2) — the "needs vetting" review queue: combine with high-reach sorting in Top sources to find big-but-unverified outlets worth escalating to a third-party trust check. `Contested` (Decision 34) filters to domains whose CURRENT entity verdict is `contested_signals` (independent sources disagree) — resolved by a backend subquery on the normalized domain, i.e. live truth rather than mention snapshots.
