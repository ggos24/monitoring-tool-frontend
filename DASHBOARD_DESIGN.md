...
- Each source row, left → right: favicon (16×16, lazy-loaded from `google.com/s2/favicons`), country flag (emoji, 14px), domain name (mono, text-secondary), then a right cluster `count + DomainScoreBadge (Trust) + ReachBadge (Reach)` in fixed-width columns (gap-4) so numbers align with their headers. Count uses **mono medium, text-primary, tabular-nums** so it reads as the row's data anchor (faint counts got lost on dark cards).
